...
AVAILABLE_SOUNDS = ["baby crying", "doorbell ringing", "smoke alarm", "fire alarm", "police siren", "ambulance siren", "car horn"]
MODES = {
...
# dshboard
# ----------------------
alerts_feed = []  # live alert feed (simulate in-memory)
...
def history():
    if "user_id" not in session:
        return redirect("/login")
    # For demo, we simulate history
    return render_template("history.html", alerts=alerts_feed)

# ----------------------
# Simulate Alert for tsa demo take out later
# ----------------------
